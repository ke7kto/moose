...
void
PMCMCDecisionVector::computeEvidence(std::vector<Real> & evidence, const DenseMatrix<Real> & input_matrix)
{
...
PMCMCDecisionVector::execute()
{
  ...
  for (auto & partial_vec: _output_value){
    _outputs_required.insert(_outputs_required.end(), partial_vec.begin(), partial_vec.end());
  }
