...
{
  InputParameters params = LikelihoodFunctionVectorBase::validParams();
                         ...
                         const bool & log_likelihood)
{
  Real result = 0.0;
  Real val1;
  mooseAssert(exp.size() == model.size(),
              "Vectors holding experiment and model conditions must be equal.");
  for (unsigned i = 0; i < exp.size(); ++i)
  {
